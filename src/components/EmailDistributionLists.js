...
function EmailDistributionLists({ onDataChange }) {
  const { token } = useAuth();
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [listsRes, contactsRes] = await Promise.all([
        fetch('http://127.0.0.1:8000/email-distribution-lists/', { headers: { 'Authorization': `Bearer ${token}` } }),
        fetch('http://127.0.0.1:8000/email-contacts/', { headers: { 'Authorization': `Bearer ${token}` } })
      ]);
  ...
  const handleSave = async () => {
    const url = editingList
      ? `http://127.0.0.1:8000/email-distribution-lists/${editingList.id}`
      : 'http://127.0.0.1:8000/email-distribution-lists/';
  ...
  const handleConfirmDelete = async () => {
    try {
      const response = await fetch(`http://127.0.0.1:8000/email-distribution-lists/${listToDeleteId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Typography variant="h5" component="h1" gutterBottom>
        Mis Listas de Distribución de Correo
      </Typography>
      <Button
        variant="contained"
        startIcon={<AddIcon />}
...
export default EmailDistributionLists;
